# -*- coding: utf-8 -*-
"""
Created on Wed Jan 12 16:17:19 2022

@author: apadelopoulou
"""
...
def AllServedPairsExhange(route_list, cost_matrix, pairlist, pairserved): 
    ...
    for pair in pairlist:       
        if pair.customers[0].added == False and pair.customers[1].added == False:
            for pserved in pairserved:
                 if pserved.customers[0].added == True and pserved.customers[1].added == True:
                     if pair.totalProfit >  pserved.totalProfit: 
                         route1=findRoute(pserved.customers[0],route_list)
                         route2=findRoute(pserved.customers[1],route_list)
                         if (route1.capacity - pserved.customers[0].demand + pair.customers[0].demand  > 150) and (route2.capacity - pserved.customers[0].demand + pair.customers[0].demand  > 150):
                             #print("CAPACITY ISSUE")
                             continue                        
                         if (route1.capacity - pserved.customers[1].demand + pair.customers[1].demand  > 150) and (route2.capacity - pserved.customers[1].demand + pair.customers[1].demand  > 150):
                             #print("CAPACITY ISSUE")
                             continue                       
                         
                         if (route1.time - pserved.customers[0].serv_time + pair.customers[0].serv_time  > 200) and (route2.time - pserved.customers[0].serv_time + pair.customers[0].serv_time > 200):
                             #print("CAPACITY ISSUE")
                             continue
                        
                         if (route1.time - pserved.customers[1].serv_time + pair.customers[1].serv_time  > 200) and (route2.time - pserved.customers[1].serv_time + pair.customers[1].serv_time  > 200):
                             #print("CAPACITY ISSUE")
                             continue
                         
                         candidateroute1 = route1.route[1:len(route1.route)-1]   
                         candidateroute2 = route2.route[1:len(route2.route)-1]                                                 
                         candidateroute1.remove(pserved.customers[0])
                         candidateroute2.remove(pserved.customers[1])
                         candidateroute3=candidateroute1
                         candidateroute4=candidateroute2
                         candidateroute1.append(pair.customers[0])
                         candidateroute2.append(pair.customers[1])
                         candidateroute3.append(pair.customers[1])
                         candidateroute4.append(pair.customers[0])
                         cand_list=[]                         
                         cand_list.append(candidateroute1)
                         cand_list.append(candidateroute2)
                         cand_list.append(candidateroute3)
                         cand_list.append(candidateroute4)                         
                         newrt = getEmptyRoutes(1)[0]
                         for c in candidateroute2:
                             c.added = False
                         for j in range(0,len(candidateroute2)):
                             best_inser = IdentifyMinimumCostInsertionInRoute(newrt,candidateroute2, cost_matrix)
                             ApplyInsertion(newrt, best_inser)
                         pair.customers[0].added = False
                         pair.customers[1].added = False
                         profit =  pair.totalProfit - pserved.totalProfit
...
def findRoute(customer,r_list):
    for r in r_list:
        if (customer in r.route):
            return r
...
def solveProblem():
    ...
    AllServedPairsExhange(route_list, cost_matrix,candidates2,allservedpairs)
